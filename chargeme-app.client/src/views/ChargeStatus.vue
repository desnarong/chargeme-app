...
<template>
        ...
        <div class="battery">
            ...
            <div class="battery-body">
                ...
                <div v-if="startLevel < fullLevel" class="lightning-icon">
                    <img v-if="currentLevel > 0" src="../assets/lightning.png" alt="Logo" class="w-[30%] ml-[35%]"
                        style="transform: rotate(270deg);"> <!-- ปรับขนาดโลโก้ได้ตามต้องการ -->
                </div>
            </div>
        </div>
        <div class="text-blue-400 font-semibold mt-[-50px]">
            <span>Power {{ currentChargeKw }} kW | SOC {{ stateOfCharge }}%</span>
            <h1 class="text-xl mt-5 text-center">{{ chargestatus }}</h1>
        </div>

        <!-- User Name -->

        <h1 class="text-[18px] text-blue-700 font-semibold mt-[80px]">{{ chargedata.charger }} {{ chargedata.header }}
        </h1>
    </div>
    <div class="grid grid-cols-2 items-center m-5">
        ...
        <div class="col-span-2 m-2 text-right">
            ...
            <p class="text-[16px] text-blue-400 font-semibold m-2">{{ cost }} {{ translations.CHARGE_STATUS_BAHT_TEXT }}
            </p>
        </div>
        ...
        <div class="col-span-2 m-2">
            ...
            <p class="text-[16px] text-blue-400 m-2">จำนวนหน่วย</p>
        </div>
        <div class="col-span-2 m-2 text-right">
            <p class="text-[16px] text-blue-400 font-semibold m-2">{{ paydata.data.orderNo }}</p>
            <p class="text-[16px] text-blue-400 font-semibold m-2">{{ paydata.data.total }} {{
                translations.CHARGE_STATUS_BAHT_TEXT }}</p>
            <p class="text-[16px] text-blue-400 font-semibold m-2">{{ transdata.fPreMeter }} kWh</p>
        </div>
...
export default {
    ...
    methods: {
        debugClick() { console.log('Root clicked!'); },
        testClick() { console.log('Button clicked!'); },
        async goBack() {
        ...
        async checkTransactionStatus() {
            try {
                while (this.currentLevel < this.fullLevel) {
                    ...
                    this.currentChargeKw = data.fCurrentChargeKw;
                    this.stateOfCharge = data.fStateOfCharge;
                    ...
                    if (data.status === 'Success' || this.transdata.fTransactionStatus === 'Finishing') {

                        if(this.transdata.fTransactionStatus === 'Finishing'){
                            setTimeout(() => this.$router.push({ name: 'finish' }), 500);
                        }
                        break;
                    }
                ...
                if (this.transdata.fTransactionStatus == 'Finishing') {
                    Cookies.remove('userCookie-' + this.chargestatusdata.fCode);
                    setTimeout(() => this.$router.push({ name: 'finish' }), 500);
                }
            } catch (error) {
        ...
        async stopTransaction() {
            try {
                ...
                const response = await axios.post(`/transaction/stop`, {
                    refNo: this.paydata.data.orderNo,
                    meterEnd: this.chargestatusdata.fCurrentMeter,
                }, {
                    headers: { Authorization: `Bearer ${this.token}` },
                });
        ...
        updateChargeTime() {
            ...
            if (this.transdata.fStartTime && this.chargestatusdata.fCurrentMeterTime) {
                const start = new Date(this.transdata.fStartTime);
                const current = new Date(this.chargestatusdata.fCurrentMeterTime);
...
.battery {
    ...
    /* หมุนแบตเตอรี่ไปทางซ้าย */
}

.battery-head {
    position: absolute;
    top: -18px;
    /* ระยะห่างจากแบตเตอรี่ */
    left: 40%;
    /* กำหนดให้เป็นศูนย์กลาง */
    width: 20px;
    /* ความกว้างของหัวแบตเตอรี่ */
    height: 10px;
    /* ความสูงของหัวแบตเตอรี่ */
...
.battery-body {
    ...
    position: relative;
    display: flex;
    /* ใช้ flexbox เพื่อให้ชิ้นส่วนเรียงกัน */
    flex-direction: column-reverse;
    /* ตั้งค่าให้เรียงจากซ้ายไปขวา */
    justify-content: space-between;
    /* ให้มีระยะห่างระหว่างชิ้น */
}

.battery-level {
    flex: 1;
    /* ให้แต่ละชิ้นมีความกว้างเท่าๆ กัน */
    height: 18%;
    /* ให้เต็มความสูง */
...
.battery-level:last-child {
    border-right: none;
    /* ไม่มีเส้นขอบด้านขวาของชิ้นสุดท้าย */
}

.battery-level.filled {
    background-color: #6cc24a;
    /* สีระดับแบตเตอรี่ที่เต็ม */
}

.lightning-icon {
    position: absolute;
    /* ทำให้ไอคอนอยู่เหนือแบตเตอรี่ */
    top: 10%;
    /* อยู่กึ่งกลางแนวตั้ง */
    transform: translate(0, -50%);
    /* ปรับตำแหน่งให้ตรงกลางในแนวตั้ง */
}
</style>
