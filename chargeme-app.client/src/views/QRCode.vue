<template>
    <div class="flex flex-col items-center">
        <div ref="qrContainer" class="rounded-lg p-2">
            <div ref="qrContainer" class="border border-gray-300 rounded-lg flex flex-col items-center mt-3 mr-5 ml-5 pb-3">
                <!-- Logo -->
                <div>
                    <img src="../assets/thai_qr_payment.png" alt="Logo" class="w-[100%] mx-auto rounded-lg flex flex-col items-center" />
                </div>
                ...
                <h1 class="text-[16px] mt-2 text-gray-500">{{ paydata.data.detail }}</h1>
                <h1 class="text-[14px] text-gray-500">{{ translations.QRCODE_ACCOUNT_TEXT }}: {{ paydata.data.stationName }}</h1>
                <p class="text-[14px] mt-2 text-lg mb-2 text-red-600" v-if="countdown && !isSuccess && showCountdown">{{ countdown }}</p>
            </div>
        </div>
        <!--<a v-if="!isExpired" @click="takeScreenshot" class="text-blue-600">
        {{ translations.QRCODE_SAVEIMAGE_TEXT }}
    </a>-->
        <p class="text-[14px] text-blue-400">{{ translations.QRCODE_SAVEIMAGE_TEXT }}</p>
        <p class="text-[14px] text-blue-400">{{ translations.QRCODE_SAVEIMAGE_TEXT2 }}</p>
        ...
        <h1 class="text-[18px] text-blue-800 font-semibold mb-4">{{ translations.QRCODE_TOTAL_TEXT }} {{ paydata.data.total }} {{ translations.QRCODE_BAHT_TEXT }}</h1>
        <h1 class="text-[18px] text-blue-800 font-semibold mb-4">{{ translations.NUMBER_OF_UNITS }} {{ paydata.data.total }} kWh</h1>
