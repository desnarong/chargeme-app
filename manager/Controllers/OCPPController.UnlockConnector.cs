...
namespace manager.Controllers
{
    public partial class OCPPController
    {
        ...
        public async Task<IActionResult> UnlockConnector2ChargePoint(string id, int connectorid)
        {
            ...
            _logger.LogTrace("UnlockConnector: Request to restart chargepoint '{0}'", id);
            UnlockConnectorRequest request = new UnlockConnectorRequest();
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        _logger.LogWarning("UnlockConnector: No API-Key configured!");
                    }
                    ...
                    {
                        _logger.LogError("UnlockConnector: Result of API  request => httpStatus={0}", response.StatusCode);
                        //httpStatuscode = (int)HttpStatusCode.OK;
            ...
            {
                _logger.LogError(exp, "UnlockConnector: Error in API request => {0}", exp.Message);
                //httpStatuscode = (int)HttpStatusCode.OK;
                //resultContent = "An error has occurred.";
            }
