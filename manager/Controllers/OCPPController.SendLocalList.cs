    ...
    public partial class OCPPController
    {
        ...
        public IActionResult GetSendLocalListJson(string[] IdTags , int version , string updatetype, string status)
        {
            ...
            foreach (string IdTag in IdTags)
            {
                ...
                list.IdTagInfo.ExpiryDate = CardModel.GetChargeTag(Guid.Parse(IdTag)).FExpiryDate.Value;
        ...
        public async Task<IActionResult> SendLocalList2ChargePoint(string id, string[] IdTags, int version, string updatetype, string status)
        {
            ...
            _logger.LogTrace("SendLocalList: Request to restart chargepoint '{0}'", id);
            SendLocalListRequest request = new SendLocalListRequest();
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        _logger.LogWarning("SendLocalList: No API-Key configured!");
                    }
                    ...
                    {
                        _logger.LogError("SendLocalList: Result of API  request => httpStatus={0}", response.StatusCode);
                        //httpStatuscode = (int)HttpStatusCode.OK;
            ...
            {
                _logger.LogError(exp, "SendLocalList: Error in API request => {0}", exp.Message);
                //httpStatuscode = (int)HttpStatusCode.OK;
                //resultContent = "An error has occurred.";
            }
