...
namespace manager.Controllers
{
    public partial class OCPPController
    {
        ...
        public async Task<IActionResult> GetLocalListVersion2ChargePoint(string id, int version)
        {
            ...
            _logger.LogTrace("GetLocalListVersion: Request to restart chargepoint '{0}'", id);
            GetLocalListVersionRequest request = new GetLocalListVersionRequest();
            request.ListVersion = version;
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        _logger.LogWarning("GetLocalListVersion: No API-Key configured!");
                    }
                    ...
                    {
                        _logger.LogError("GetLocalListVersion: Result of API  request => httpStatus={0}", response.StatusCode);
                        //httpStatuscode = (int)HttpStatusCode.OK;
            ...
            {
                _logger.LogError(exp, "GetLocalListVersion: Error in API request => {0}", exp.Message);
                //httpStatuscode = (int)HttpStatusCode.OK;
                //resultContent = "An error has occurred.";
            }
