...
namespace manager.Controllers
{
    public class TransactionsController : Controller
    {
        ...
        public IActionResult GetChargersSelect()
        {
            ...
            if (userInfo.FCompanyId == Guid.Empty)
            {
                var data = ChargerModel.GetConnectorStatusViews();
                data = data.OrderBy(x => x.ShortName).ThenBy(x => x.ConnectorId).ToList();
                return new JsonResult(new { data = data });
            }
            else
            {
                var data = ChargerModel.GetConnectorStatusViews(userInfo.FCompanyId);
                data = data.OrderBy(x => x.ShortName).ThenBy(x => x.ConnectorId).ToList();
                return new JsonResult(new { data = data });
            }
        }
        ...
        {
            var data = CardModel.GetChargeTag(Guid.Parse(chargetagid));
            return new JsonResult(new { data = data });
        }
