...
namespace manager.Controllers
{
    public class DashboardsController : Controller
    {
        ...
        public DashboardsController(IConfiguration configuration, ILogger<DashboardsController> logger)
        {
            _logger = logger;
        ...
        public IActionResult ConnecterDetail(string Charger, string Connecter)
        {
            var model = new ConnectorStatusDataModel();
            model.connectorStatusViewDatas = ChargerModel.GetConnectorStatusViewDatas(Guid.Parse(Charger)).Where(x => x.Id == Guid.Parse(Connecter)).ToList();
            return View(model);
        }
        public IActionResult GetChargeLogsTable(string ChargePoint, string ConectorId)
        {
            ...
            var model = ChargerModel.GetMessageLogDatasWithRows(Guid.Parse(ChargePoint), Guid.Parse(ConectorId), rows);
            switch (order)
            {
                case "0":
