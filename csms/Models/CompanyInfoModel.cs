            ...
            using (var context = new NpgsqlDbContext())
            {
                var data = (from company in context.TblCompanies
                            select new CompanyInfoData
                            {
                                ...
                                Logo = company.FLogo,
                                StatusView = $"<a></>",
                                ActionEdit = $"<a href='JavaScript:EditStationModal(\"{company.FId}\")' class='btn btn-clean btn-icon btn-icon-md' title='edit'><i class='fa fa-edit'></i></a>",
                                ActionDelete = $"<a href='JavaScript:DeleteStationModalClick(\"{company.FId}\")' class='btn btn-clean btn-icon btn-icon-md' title='delete'><i class='fa fa-trash'></i></a>"
                            })
                            .ToList();
