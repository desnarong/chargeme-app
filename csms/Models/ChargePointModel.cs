...
namespace csms.Models
{
    public class ChargePointModel
    {
        ...
        public static List<TblCharger> GetChargePoints()
        {
            ...
            using (var context = new NpgsqlDbContext())
            {
                var data = (from item in context.TblConnectorStatuses
                            ...
                            select new ConnectorStatusData
                            {
                                ...
                                ChargePointName = chargepoint.FName,
                                ConnectorId = item.FConnectorId ?? 0,
                                ConnectorName = item.FName,
                                Code = item.FCode,
                                IsOnline = (item.FCurrentStatus == "" ? "<i class='fa-sharp fa-cloud fa-2x color-offline'></i>" : "<i class='fa-sharp fa-cloud fa-2x color-online'></i>"),
                                ...
                                ActionView = $"<a href='/Home/Detail?ChargePoint={item.FChargerId}&ConectorId={item.FId}' class='btn btn-clean btn-icon btn-icon-md' title='View'><i class='fa fa-eye'></i></a>",
                                ActionEdit = $"<a href='JavaScript:EditConectorModal(\"{item.FChargerId}\",\"{item.FId}\")' class='btn btn-clean btn-icon btn-icon-md' title='edit'><i class='fa fa-edit'></i></a>",
                            }).ToList();
                return data;
            }
        }
                ...
                try
                {
                    var query = from cs in context.ConnectorStatusViews
                                ...
                                select new ConnectorStatusViewData()
                                {
                                    ...
                                    ChargePointName = cs.FChargerName,
                                    ShortName = cs.FShortName,
                                    Id = cs.FId,
                                    ConnectorCode = cs.FCode,
                                    ConnectorId = cs.FConnectorId,
                                    ConnectorName = cs.FName,
                                    IsOnline = "<i class='fa-sharp fa-cloud fa-2x color-offline'></i>", //(cs.FCurrentStatus == "" ? "<i class='fa-sharp fa-cloud fa-2x color-offline'></i>" : "<i class='fa-sharp fa-cloud fa-2x color-online'></i>"),
                                    ...
                                    ActionView = $"<a href='/Home/Detail?Charger={cs.FChargerId}&ConectorId={cs.FId}' class='btn btn-clean btn-icon btn-icon-md' title='View'><i class='fa fa-eye'></i></a>",
                                    //ActionEdit = $"<a href='JavaScript:EditConectorModal(\"{item.ChargerId}\",\"{item.ConnectorId}\")' class='btn btn-clean btn-icon btn-icon-md' title='edit'><i class='fa fa-edit'></i></a>",
                                };
                    data = query.ToList();
                }
                catch(Exception ee)
                ...
                try
                {
                    var query = from cs in context.ConnectorStatusViews
                                ...
                                select new ConnectorStatusViewData()
                                {
                                    ...
                                    ConnectorId = cs.FConnectorId,
                                    ConnectorName = cs.FName,
                                    IsOnline = (cs.FCurrentStatus == "" ? "<i class='fa-sharp fa-cloud fa-2x color-offline'></i>" : "<i class='fa-sharp fa-cloud fa-2x color-online'></i>"),
                                    LastStatus = cs.FCurrentStatus,
                                    LastStatusTime = TimeZoneInfo.ConvertTimeFromUtc(cs.FCurrentStatusTime.Value, timeZone),
                                    LastMeter = Convert.ToDouble(cs.FCurrentMeter),
                                    LastMeterTime = TimeZoneInfo.ConvertTimeFromUtc(cs.FCurrentMeterTime.Value, timeZone),
        ...
        //public static List<ConnectorStatusViewData> GetConnectorStatusViewDatas()
        //{
        //    var data = new List<ConnectorStatusViewData>();
        //    using (var context = new NpgsqlDbContext())
        //    {
        //        //on new { cs.FStationId, cs.FConnectorId } equals new { t.FStationId, t.FConnectId }
        //        //.Where(subT => subT.FStationId == cs.FStationId && subT.FConnectStatusId == cs.FConnectorId)
        //        var query = from cs in context.TblConnectorStatuses.AsNoTracking()
        //                    join cg in context.TblChargers on cs.FChargerId equals cg.FId
        //                    join tmp_transaction in context.TblTransactions on 
        //                        new { cs.FStationId, cs.FId } equals new { tmp_transaction.FStationId, tmp_transaction.FId } into transactionGroup
        //                    from t in transactionGroup.DefaultIfEmpty() // RIGHT OUTER JOIN
        //                    where t == null
        //                    /*||
        //                        context.TblTransactions
        //                        .Where(subT => subT.FStationId == cs.FStationId)
        //                        .OrderByDescending(subT => subT.FId)
        //                        .Select(subT => subT.FId)
        //                        .FirstOrDefault() == t.FId*/
        //                    select new ConnectorStatusViewData
        //                    {
        //                        ChargerId = cs.FChargerId,
        //                        ChargePointName = cg.FName,
        //                        ConnectorId = Guid.Parse( cs.FConnectorId.ToString()),
        //                        ConnectorName = cs.FName,
        //                        IsOnline = (cs.FCurrentStatus == "" ? "<i class='fa-sharp fa-cloud fa-2x color-offline'></i>" : "<i class='fa-sharp fa-cloud fa-2x color-online'></i>"),
        //                        LastStatus = cs.FCurrentStatus,
        //                        LastStatusTime = cs.FCurrentStatusTime,
        //                        LastMeter = Convert.ToDouble(cs.FCurrentMeter),
        //                        LastMeterTime = cs.FCurrentStatusTime,
        //                        StateOfCharge = Convert.ToDouble(cs.FStateOfCharge),
        //                        CurrentChargeKw = Convert.ToDouble(cs.FCurrentChargeKw),
        //                        //Comment = chargepoint.Comment,
        //                        //Image = chargepoint.Image,
        //                        TransactionId = t.FId,
        //                        ActionView = $"<a href='/Home/Detail?ChargePoint={cg.FId}&ConectorId={cs.FConnectorId}' class='btn btn-clean btn-icon btn-icon-md' title='View'><i class='fa fa-eye'></i></a>",
        //                        //ActionEdit = $"<a href='JavaScript:EditConectorModal(\"{item.ChargerId}\",\"{item.ConnectorId}\")' class='btn btn-clean btn-icon btn-icon-md' title='edit'><i class='fa fa-edit'></i></a>",
        //                    };
        //        return query.ToList();
        //    }
        ...
        public static List<ChargePointData> GetChargePointDatas()
        {
            using (var context = new NpgsqlDbContext())
            {
                var data = (from item in context.TblChargers
                            ...
                            select new ChargePointData
                            {
                                ...
                                Image = item.FImage,
                                ActionEdit = $"<a href='JavaScript:EditChargePointModal(\"{item.FId}\")' class='btn btn-clean btn-icon btn-icon-md' title='edit'><i class='fa fa-edit'></i></a>",
                                ActionDelete = $"<a href='JavaScript:DeleteChargePointModalClick(\"{item.FId}\")' class='btn btn-clean btn-icon btn-icon-md' title='delete'><i class='fa fa-trash'></i></a>"
                            })
                             .ToList();
                return data;
            }
        }
        public static TblCharger GetChargePoint(Guid chargepointid)
        {
            using (var context = new NpgsqlDbContext())
            {
        ...
        public static TblCharger CreateChargePoint(TblCharger model)
        {
            using (var context = new NpgsqlDbContext())
            {
                context.Add(model);
        ...
        public static TblCharger UpdateChargePoint(TblCharger model)
        {
            using (var context = new NpgsqlDbContext())
            {
        ...
        public static string DeleteChargePoint(TblCharger model)
        {
            using (var context = new NpgsqlDbContext())
            {
    ...
    public partial class ChargePointData
    {
