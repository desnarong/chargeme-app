            ...
            try
            {
                ...
                if (ChargePointStatus != null)
                {
                    ...
                    foreach (MeterValue meterValue in meterValueRequest.MeterValue)
                    {
                        ...
                        {
                            Logger.LogTrace("MeterValues => ChargePoint={7} / Connector={8} Context={0} / Format={1} / Value={2} / Unit={3} / Location={4} / Measurand={5} / Phase={6}",
                                ...
                                ChargePointStatus?.Name, connectorId);
                            ...
                            if (sampleValue.Measurand == SampledValueMeasurand.Power_Active_Import)
                            {
                                ...
                                if (double.TryParse(sampleValue.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out currentChargeKW))
                                {
                                    ...
                                    {
                                        Logger.LogTrace("MeterValues => ChargePoint={1} / Connector={2} Charging '{0:0.0}' W", currentChargeKW, ChargePointStatus?.Name, connectorId);
                                        // convert W => kW
                                    ...
                                    {
                                        // already kW => OK
                                        Logger.LogTrace("MeterValues => ChargePoint={1} / Connector={2} Charging '{0:0.0}' kW", currentChargeKW, ChargePointStatus?.Name, connectorId);
                                    }
                                    else
                                    {
                                        Logger.LogWarning("MeterValues => ChargePoint={2} / Connector={3} Charging: unexpected unit: '{0}' (Value={1})", sampleValue.Unit, sampleValue.Value, ChargePointStatus?.Name, connectorId);
                                    }
                                }
                                else
                                {
                                    Logger.LogError("MeterValues => ChargePoint={1} / Connector={2} Charging: invalid value '{0}' (Unit={1})", sampleValue.Value, sampleValue.Unit, ChargePointStatus?.Name, connectorId);
                                }
                            }
                                    ...
                                    sampleValue.Measurand == null)
                            {
                                ...
                                if (double.TryParse(sampleValue.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out meterKWH))
                                {
                                    if (sampleValue.Unit == SampledValueUnit.Wh ||
                                        ...
                                        sampleValue.Unit == null)
                                    {
                                        Logger.LogTrace("MeterValues => ChargePoint={1} / Connector={2} Value: '{0:0.0}' Wh", meterKWH, ChargePointStatus?.Name, connectorId);
                                        // convert Wh => kWh
                                    ...
                                    {
                                        // already kWh => OK
                                        Logger.LogTrace("MeterValues => ChargePoint={1} / Connector={2} Value: '{0:0.0}' kWh", meterKWH, ChargePointStatus?.Name, connectorId);
                                    }
                                    else
                                    {
                                        Logger.LogWarning("MeterValues => ChargePoint={2} / Connector={3} Value: unexpected unit: '{0}' (Value={1})", sampleValue.Unit, sampleValue.Value, ChargePointStatus?.Name, connectorId);
                                    }
                                    meterTime = meterValue.Timestamp;
                                }
                                else
                                {
                                    Logger.LogError("MeterValues => ChargePoint={2} / Connector={3} Value: invalid value '{0}' (Unit={1})", sampleValue.Value, sampleValue.Unit, ChargePointStatus?.Name, connectorId);
                                }
                            }
                            else if (sampleValue.Measurand == SampledValueMeasurand.SoC)
                            {
                                ...
                                {
                                    Logger.LogTrace("MeterValues => ChargePoint={1} / Connector={2} SoC: '{0:0.0}'%", stateOfCharge, ChargePointStatus?.Name, connectorId);
                                }
                                else
                                {
                                    Logger.LogError("MeterValues => ChargePoint={1} / Connector={2} invalid value '{0}' (SoC)", sampleValue.Value, ChargePointStatus?.Name, connectorId);
                                }
                            }
                        }
                    }
