...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, NpgsqlDbContext dbContext)
        {
            ...
            if (context.Request.Path.StartsWithSegments("/OCPP"))
            {
                ...
                {
                    TblCharger chargePoint = dbContext.TblChargers.Where(x => x.FShortName.Equals(chargepointIdentifier)).FirstOrDefault();
                    if (chargePoint != null)
                    {
                        _logger.LogInformation("OCPPMiddleware => SUCCESS: Found chargepoint with identifier={0}", chargePoint.FId);

                        // Check optional chargepoint authentication
                        if (!string.IsNullOrWhiteSpace(chargePoint.FUsername))
                        {
                            ...
                            if (!string.IsNullOrEmpty(authHeader))
                            {
                                ...
                                if (cred.Length == 2 && chargePoint.FUsername == cred[0] && chargePoint.FPassword == cred[1])
                                {
                                    // Authentication match => OK
                                    _logger.LogInformation("OCPPMiddleware => SUCCESS: Basic authentication for chargepoint '{0}' match", chargePoint.FId);
                                    basicAuthSuccess = true;
                                }
                                else
                                {
                                    // Authentication does NOT match => Failure
                                    _logger.LogWarning("OCPPMiddleware => FAILURE: Basic authentication for chargepoint '{0}' does NOT match", chargePoint.FId);
                                }
                            }
                        ...
                        else if (!string.IsNullOrWhiteSpace(chargePoint.FClientCertThumb))
                        {
                            // Chargepoint MUST send basic authentication header
                            ...
                            if (clientCert != null)
                            {
                                if (clientCert.Thumbprint.Equals(chargePoint.FClientCertThumb, StringComparison.InvariantCultureIgnoreCase))
                                {
                                    // Authentication match => OK
                                    _logger.LogInformation("OCPPMiddleware => SUCCESS: Certificate authentication for chargepoint '{0}' match", chargePoint.FId);
                                    certAuthSuccess = true;
                                }
                                else
                                {
                                    // Authentication does NOT match => Failure
                                    _logger.LogWarning("OCPPMiddleware => FAILURE: Certificate authentication for chargepoint '{0}' does NOT match", chargePoint.FId);
                                }
                            }
                        ...
                        {
                            _logger.LogInformation("OCPPMiddleware => No authentication for chargepoint '{0}' configured", chargePoint.FId);
                        }
                        ...
                        else
                        {
                                ...
                                lock (_chargePointStatusDict)
                                {
                                    ...
                                    if (_chargePointStatusDict.ContainsKey(chargepointIdentifier))
                                    {
                                        ...
                                        if (_chargePointStatusDict[chargepointIdentifier].WebSocket.State != WebSocketState.Open)
                                        {
                                            ...
                                            _chargePointStatusDict.TryRemove(chargepointIdentifier, out _); // ลบออกเมื่อปิดการเชื่อมต่อ
                                        }
                                    }
                            ...
                            if (statusSuccess)
                            {
                                ...
                                using (WebSocket webSocket = await context.WebSockets.AcceptWebSocketAsync(subProtocol))
                                {
                                    ...
                                    _ = PingClient(webSocket); // ส่ง Ping ป้องกัน Timeout
                    ...
                    if (cmd == "Status")
                    {
                        try
                        {
                            ...
                            foreach (ChargePointStatus status in _chargePointStatusDict.Values)
                            {
                                ...
                                //}
                                if (status.WebSocket != null)
                                {
                                    status.WebSocketStatus = status.WebSocket.State.ToString();

                                    if (status.WebSocketStatus != "Open")
                                    {
                                        foreach (int key in status.OnlineConnectors.Keys)
                                        {
                                            status.OnlineConnectors[key].Status = Models.ConnectorStatusEnum.Undefined;
                                        }
                                    }
                                }
                        ...
                        if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                        {
                            switch (cmd)
                            {
                                ...
                                case "RemoteStopTransaction":
                                    ...
                                    {
                                        // OCPP V1.6
                                        await RemoteStopTransaction16(status, context, dbContext);
                                    }
                                    break;
                                case "ClearCache":
                                    // Send message to chargepoint
        ...
        // ส่งข้อความไปยังทุกเครื่องที่เชื่อมต่อ
        private async Task BroadcastMessage(string message)
        {
            foreach (var client in _chargePointStatusDict.Values)
            {
        ...
        private async Task PingClient(WebSocket webSocket)
        {
            while (webSocket.State == WebSocketState.Open)
            {
                await webSocket.SendAsync(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new { status = "Ping" })), WebSocketMessageType.Text, true, CancellationToken.None);
                await Task.Delay(30000); // ส่งทุก 30 วินาที
            }
        }
        ...
        {
            if (!_configuration.GetValue<bool>("MessageDumpDirUse")) return;

            string dumpDir = _configuration.GetValue<string>("MessageDumpDir");
            if (!string.IsNullOrWhiteSpace(dumpDir))
            {
                string path = Path.Combine(dumpDir, string.Format("{0}_{1}.txt", DateTime.UtcNow.ToString("yyyy-MM-dd_HH-mm-ss-ffff"), nameSuffix));
                try
                {
