...
namespace OCPP.Core.Server
{
    public class Startup
    {
        ...
        {
            var connectionString = Configuration.GetConnectionString("NpgServer");
            //services.AddOCPPDbContext(Configuration);
            services.AddDbContextFactory<NpgsqlDbContext>(options =>
            {
                            ...
                            IServiceScopeFactory serviceScopeFactory)
        {
            ...
            var retentionDays = 7;
            // Initial cleanup
            CleanupOldLogFiles.CleanupOldLog(logDirectory, retentionDays);
            // Set up a recurring cleanup task (optional)
            var cleanupTimer = new System.Threading.Timer(
                _ => CleanupOldLogFiles.CleanupOldLog(logDirectory, retentionDays),
                null,
                TimeSpan.FromDays(1), // Start after 1 day
